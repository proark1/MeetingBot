...
Event-loop hygiene and repository cleanup.

### Fixed
- **Blocking `time.sleep` on the event loop** — the async bot-run coroutine in `browser_bot.py` used a bare `time.sleep(0.5)` while starting the video-recording ffmpeg process, freezing every other concurrent bot *and* all API requests served by the same event loop for half a second. Changed to `await asyncio.sleep(0.5)`. (Verified via AST scan that this was the only `time.sleep` reachable from an async context — the `_start_pulseaudio` / `_start_xvfb` / `_start_ffmpeg` helpers run in worker threads and are unaffected.)

### Internal
- Repo is now `ruff`-clean across `app/` **and** `tests/` (removed 7 unused imports in the test suite).
- Removed a stray duplicate `backend/openapi.json` accidentally committed in 2.63.0 — the canonical snapshots live at `api/openapi.json` and `api/openapi.admin.json` (generated by `scripts/generate_openapi.py`).
