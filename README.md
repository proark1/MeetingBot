...
**Version 2.58.0** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface with 100% example coverage — every one of the 114 public + 133 admin operations now has summary, description, tags, request example (where applicable), and a 2xx response example**.

> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.58.0 · **Build:** Join-reliability metrics (self-healing-join observability) — new Prometheus counters `meetingbot_bot_join_attempts_total{platform}` and `meetingbot_bot_join_results_total{platform,result}` instrument the existing per-bot join retry loop, so selector drift shows up as a rising failure rate per platform (pairs with the synthetic canary). Also wired the previously-dead `bots_created`/`bots_completed` lifecycle counters. Previous build (2.57.1): GDPR erasure fix — `DELETE /api/v1/auth/account` now purges *every* per-account table reflectively (the previous hand-maintained list silently left `meeting_summaries` and `retention_policies` behind); `audit_logs` are intentionally retained for traceability. Previous build (2.57.0): Efficiency + operational observability + CI quality gate. Efficiency: per-bot diagnostic loops mutate their sample buffers in place instead of copy + `store.update_bot` every 15s (removes global-lock contention that scaled with bot count); `cleanup_expired` deletes recording/video files off the event loop. Ops: optional Sentry error tracking (`SENTRY_DSN`, graceful no-op otherwise); the synthetic canary now runs from the app lifespan when `CANARY_ENABLED=true`, exercising the join→audio→transcript→leave pipeline to catch `browser_bot.py` selector drift; a ruff lint CI job (bug-catching rules) plus `backend/ruff.toml`. Fixes: quota reservation now fails closed (rollback + 503) on DB error instead of running the bot un-metered; removed unused imports and a duplicate `Response` import. Previous build (2.56.0): security/correctness/performance hardening pass — SSO account-takeover fix, OAuth state cookie-binding, SAML signed-assertion enforcement + SSRF-guarded metadata, dedicated `ENCRYPTION_KEY`, event-loop unblock at bot launch, idempotent billing, WS tenant isolation, analysis→Sonnet + transcript caching. <!-- auto-updated on each release -->
