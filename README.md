...
**Version 2.56.0** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface with 100% example coverage — every one of the 114 public + 133 admin operations now has summary, description, tags, request example (where applicable), and a 2xx response example**.

> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.56.0 · **Build:** Hardening pass — security, correctness, performance, and efficiency fixes across the service following a full audit. Security: enforce provider email-verification before SSO auto-links to an existing account (account-takeover fix), bind OAuth `state` to a single-use browser cookie + full-length HMAC, enforce SAML signed-assertion settings + SSRF-guard IdP-metadata fetches, separate encryption-at-rest key (`ENCRYPTION_KEY`) from `JWT_SECRET`, rate-limit SSO callbacks, and stop leaking upstream exception strings. Correctness: stop blocking the event loop during bot launch (PulseAudio/ffmpeg/Xvfb now off-thread), make per-bot credit deduction idempotent, fix a WebSocket cross-tenant broadcast leak, log swallowed background-task exceptions, and bound the keyword-alert cache. Performance: route post-meeting analysis to Sonnet with transcript prompt-caching (large AI-cost reduction), tighten Chromium launch flags, reuse pooled HTTP clients on hot paths, and lazily load old terminal snapshots. Previous build (2.55.0): Distributed-state Phase 1 — a Redis-backed `BotStateStore` (`redis_store.py`) plus `BotSession.to_state_dict`/`from_state_dict` serialization (excludes process-local handles), selectable via `BOT_STATE_BACKEND=redis` + `REDIS_URL`. The in-memory store stays the default and call sites are unchanged, so the running product is untouched until the backend is wired in and validated on staging. Verified end-to-end with `fakeredis`. Previous build (2.53.0): Performance/efficiency pass — reuse Anthropic & Gemini clients/models instead of rebuilding them per call, cap and (for repeat Q&A) prompt-cache transcripts sent to the LLM, batch per-speaker sentiment and post-meeting translation into single calls, cache account keyword-alert rules per meeting instead of querying on every transcript line, fan out webhook delivery concurrently with a short-TTL SSRF DNS cache, offload CPU/blocking work off the event loop, dedupe the in-meeting chat scrape, add composite DB indexes, and stop fetching 10k bots to render a single page. <!-- auto-updated on each release --> — a Redis-backed `BotStateStore` (`redis_store.py`) plus `BotSession.to_state_dict`/`from_state_dict` serialization (excludes process-local handles), selectable via `BOT_STATE_BACKEND=redis` + `REDIS_URL`. The in-memory store stays the default and call sites are unchanged, so the running product is untouched until the backend is wired in and validated on staging. Verified end-to-end with `fakeredis` (CRUD, filtered/paginated listing, share-hash index, immutable-field guard, parity with the in-memory store). Test suite 82 → 96. Previous build (2.54.0): Groundwork for the open scaling/resilience features — introduced a `BotStateStore` protocol (the in-memory store now conforms, so a future shared/distributed backend can be swapped in without touching call sites) and a synthetic-canary skeleton (`scripts/canary.py` + `canary_service`) that joins a per-platform test meeting and asserts the join→audio→transcript→leave pipeline, the earliest warning for `browser_bot.py` selector drift. Test suite 71 → 82. Previous build (2.53.1): expanded the automated test suite (39 → 71 tests) — added unit coverage for the efficiency-pass logic: intelligence-service helpers (transcript cap, sentiment coercion, cached prompt-prefix blocks, client/model reuse), config parsing, webhook signing + SSRF/DNS cache, store `list_bots` filtering, keyword-alert caching, and admin snapshot aggregation. Corrected the stale "no tests" note in CLAUDE.md. Previous build (2.53.0): Performance/efficiency pass — reuse Anthropic & Gemini clients/models instead of rebuilding them per call, cap and (for repeat Q&A) prompt-cache transcripts sent to the LLM, batch per-speaker sentiment and post-meeting translation into single calls, cache account keyword-alert rules per meeting instead of querying on every transcript line, fan out webhook delivery concurrently with a short-TTL SSRF DNS cache, offload CPU/blocking work (admin analytics JSON parsing, PDF rendering, in-meeting audio file reads & health probe) off the event loop, dedupe the in-meeting chat scrape, add composite DB indexes for the action-item and webhook-delivery list queries, and stop fetching 10k bots to render a single page. <!-- auto-updated on each release -->
