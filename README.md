...
**Version 2.51.0** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface — request-aware `servers:`, typed `error_code` enum, formal rate-limit + Retry-After header components, examples on every core schema, ErrorResponse/WebhookEvent/WebhookPayload components, CI drift check, SDK regenerator, and an `/api/quickstart` landing page**.

> **Last updated:** 2026-05-04 · **API version in Swagger UI:** 2.51.0 · **Build:** Round-3 OpenAPI polish — `/api/openapi.json` now returns servers derived from the actual request host (so production no longer shows localhost when `PUBLIC_BASE_URL` is unset); `error_code` is now a typed enum of the 12 known codes; new `X-RateLimit-Remaining`, `X-RateLimit-Reset`, and `Retry-After` reusable header components attached to the 429 response; request examples added to `MeetingAnalysis`, `KeywordAlertCreate`, `RetentionPolicyUpdate`, `ActionItemPatch`. Custom `/api/docs`, `/api/redoc`, `/api/openapi.json` routes replace FastAPI auto-routes for full request awareness. <!-- auto-updated on each release -->
