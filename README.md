...
**Version 2.62.0** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface with 100% example coverage — every one of the 114 public + 133 admin operations now has summary, description, tags, request example (where applicable), and a 2xx response example**.

> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.62.0 · **Build:** Security + reliability — prompt injection hardened on bot_name / caption_context / meeting_url (full angle-bracket escaping + length caps), ffmpeg audio-split timeout (600s, kills on hang), per-chunk transcription timeout (600s), transcript entry count sanity check, archive search scoped to last 90 days, transcript search bounded to 200 bots, tighter rate limits on AI endpoints (analyze 5/min, followup-email 3/min), `AskRequest.question` max_length=1000 enforcement, 14 new injection guard tests. Previous build (2.61.0): Cursor pagination, bulk ops, webhook delivery history. <!-- auto-updated on each release -->
