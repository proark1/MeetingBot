...
> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.63.3 · **Build:** Reminder-scan efficiency — the action-item reminder background loop no longer re-scans items already at the terminal "overdue" stage every cycle (they can never advance), bounding the working set as open-but-overdue items accumulate. Behaviour-preserving; +2 regression tests guarding NULL-stage handling. Previous build (2.63.2): blocking `time.sleep` fix on the event loop. <!-- auto-updated on each release -->
