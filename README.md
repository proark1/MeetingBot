...
**Version 2.60.0** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface with 100% example coverage — every one of the 114 public + 133 admin operations now has summary, description, tags, request example (where applicable), and a 2xx response example**.

> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.60.0 · **Build:** Maintainability — adopted **Alembic** for schema migrations (baseline = current schema, applies cleanly to a fresh DB; coexists with the legacy in-app migrator, cutover deferred) and began decomposing the 5,807-line `browser_bot.py`: extracted the per-platform admission/end/alone text signals into `services/browser/platform_texts.py` (the documented platform extension point), re-imported under their legacy names so behavior is unchanged. Deeply-coupled page/audio logic was intentionally left in place — splitting it on an untested file is high-regression-risk for cosmetic gain. Previous build (2.59.0): Action-item due-date reminders — an opt-in background loop fires `action_item.due_soon` / `action_item.overdue` webhook events. Previous build (2.58.0): Join-reliability metrics — Prometheus counters `meetingbot_bot_join_attempts_total{platform}` and `meetingbot_bot_join_results_total{platform,result}` instrument the per-bot join retry loop so selector drift shows up as a rising failure rate (pairs with the synthetic canary); wired the previously-dead `bots_created`/`bots_completed` counters. Previous build (2.57.1): GDPR erasure fix — `DELETE /api/v1/auth/account` now purges *every* per-account table reflectively. Previous build (2.57.0): Efficiency + operational observability + CI quality gate (in-place diagnostic buffers, off-loop file cleanup, optional Sentry, the synthetic canary, ruff lint CI). Previous build (2.56.0): security/correctness/performance hardening pass. <!-- auto-updated on each release -->
