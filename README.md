...
**Version 2.63.2** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface with 100% example coverage — every one of the 114 public + 133 admin operations now has summary, description, tags, request example (where applicable), and a 2xx response example**.

> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.63.2 · **Build:** Event-loop hygiene + cleanup — fixed a blocking `time.sleep(0.5)` in the async bot-run coroutine (was freezing all concurrent bots + API requests for 0.5s when video recording started), now `await asyncio.sleep`. Repo-wide ruff clean, removed a stray duplicate OpenAPI snapshot. Previous build (2.63.1): UI top-up missing-await fix + external-call timeouts. <!-- auto-updated on each release -->
