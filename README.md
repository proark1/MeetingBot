...
**Version 2.61.0** — A stateless meeting bot API service with multi-tenant billing, business account support, Google/Microsoft SSO, Python & JS SDKs, webhook retry/delivery logs, bot persona customization, video recording, Prometheus metrics, idempotency keys, cloud storage, email notifications, calendar auto-join, Slack/Notion integrations, GDPR compliance, an opt-in advanced-features layer (in-meeting @bot Q&A, live speaker analytics, smart decision detection, cross-meeting memory, host coaching, agentic delegation), **and a fully usable OpenAPI 3.1 surface with 100% example coverage — every one of the 114 public + 133 admin operations now has summary, description, tags, request example (where applicable), and a 2xx response example**.

> **Last updated:** 2026-06-01 · **API version in Swagger UI:** 2.61.0 · **Build:** Production hardening — cursor-based pagination (`next_cursor` on `GET /api/v1/bot`), bulk cancel (`POST /api/v1/bot/bulk/cancel`) and bulk delete (`DELETE /api/v1/bot/bulk`) endpoints, per-webhook delivery history (`GET /api/v1/webhooks/{id}/deliveries`) with event/status filters, `browser/text_matchers.py` (extracted pure text-matching helpers + 80-case test suite), speaker normalisation guard, httpx connection pool limits. Previous build (2.60.0): Alembic schema migrations + `services/browser/platform_texts.py` data extraction. Previous build (2.59.0): Action-item due-date reminders. Previous build (2.58.0): Join-reliability Prometheus metrics. Previous build (2.57.1): GDPR reflective erasure. Previous build (2.57.0): Sentry, canary loop, ruff CI. <!-- auto-updated on each release -->
